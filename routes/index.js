...
var debug = require('debug')('spellah')
...
function eachCons (a, size, callback) {
  if (a.length > size) { for (var j = 0; j <= (a.length - size); j++) callback(a.slice(j, j + size)) } else { callback(a) }
}

function mapOf (value) {
  var map = {}
  if (value.results === undefined) return map
...
function check (tokens, ngrams) {
  ...
  if (levels.length > 1) {
    for (var i = 0, j = 1, len = levels.length; j < len; i++, j++) {
      var token1 = tokens[i]; var token2 = tokens[j]
...
router.get('/check', function (req, res) {
  ...
  cache.get(key, function (err, cached) {
    if (err || !cached) {
      ...
      debug('cache hit "' + req.query.q + '" => ' + cached)
